...
function pageTemplate(){
  ...
  <div class="info">
    ...
    <div>
      BILL NO: <input type="text" class="billNo" placeholder="bill no.">
    </div>
  </div>
  ...
  tr.innerHTML = `
    ...
    <td style="padding:0;">
      <div style="display:flex; width:100%; align-items:flex-start;">
        <textarea class="bold-text detail-text" style="width:60%; border-right:1px solid #000;" rows="1"></textarea>
        <input type="text" class="measure" style="width:40%;">
      </div>
    </td>
    <td><input type="text" class="sqft" inputmode="decimal"></td>
    <td><input type="text" class="qty" inputmode="decimal" value="1"></td>
...
function serializeBill(){
  const pages = [...document.querySelectorAll(".invoice-box")].map(box => {
    const rows = [...box.querySelectorAll(".item-row")].map(row => ({
      ...
      sqftManual: row.querySelector(".sqft").classList.contains("manual"),
...
function deserializeBill(data){
  ...
  (data.pages && data.pages.length ? data.pages : [{}]).forEach(pageData => {
    ...
    rows.forEach(rowData => {
      ...
      tr.querySelector(".measure").value = rowData.measure || "";
      tr.querySelector(".sqft").value = rowData.sqft || "";
      tr.querySelector(".qty").value = rowData.qty || "1";
      tr.querySelector(".rate").value = rowData.rate || "";
      tr.querySelector(".total").value = rowData.total || "";
      if (rowData.sqftManual) tr.querySelector(".sqft").classList.add("manual");
...
async function initBill(){
  ...
  if (id){
    ...
    try {
      ...
      if (data){
        ...
        alert("यह बिल अब मौजूद नहीं है। नया बिल शुरू किया जा रहा है।");
      }
    } catch (err){
      console.error(err);
  ...
  first.querySelector(".billNo").value = "BILL-" + Date.now();
  reflow();
  suppressDirty = false;
  hasUnsavedChanges = false;
  updateSaveStatus();
...
document.getElementById("pages").addEventListener("input", e => {
  ...
  if (t.matches(".measure, .sqft, .qty, .rate, .total")){
    calcRow(t.closest("tr"));
  }
  calculateGrandTotal();

  if (t.matches(".detail-text, .measure, .sqft, .qty, .rate, .total")){
    scheduleReflow();
  }
  markDirty();
});
...
function parseMeasurement(text){
  if (!text) return 0;
  const parts = String(text).split(/×|x|X|\*/);
  if (parts.length !== 2) return 0;

  const toFeet = str => {
    str = String(str).trim();
    const feetMatch    = str.match(/(\d+(?:\.\d+)?)\s*(?:ft|feet|')/i);
    const inchMatch    = str.match(/(\d+(?:\.\d+)?)\s*(?:in|inch|")/i);
  ...
  return toFeet(parts[0]) * toFeet(parts[1]);
}
...
function calcRow(row){
  ...
  const sqftInput = row.querySelector(".sqft");
...
function pdfFileName(){
  ...
  const raw = (firstBillNo && firstBillNo.value ? firstBillNo.value : "Bill").trim();
  const safe = raw.replace(/[^\w\-]+/g, "_").slice(0, 60) || "Bill";
  return safe + ".pdf";
}
